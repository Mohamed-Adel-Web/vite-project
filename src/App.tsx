...
function ARCamera() {
  ...
  useEffect(() => {
    const import { useEffect, useRef, useState } from "react";
import * as THREE from "three";
import { Canvas, useThree, useFrame } from "@react-three/fiber";
import { GLTFLoader } from "three/examples/jsm/loaders/GLTFLoader";
import { Environment } from "@react-three/drei";
...
function Model() {
  ...
  useFrame(() => {
    ...
    if (!initialized && (gyroRef.current.beta !== 0 || gyroRef.current.gamma !== 0)) {
  ...
  return (
    <group
      ...
      scale={[scale, scale, scale]}
      rotation={[0, rotation, 0]}
      position={modelWorldPosition.current ? [
...
export default function App() {
  ...
  if (!start) {
    return (
      <div style={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", backgroundColor: "black", color: "white" }}>
        <h1 style={{ fontSize: "1.875rem", marginBottom: "1.5rem" }}>🎮 AR Treasure Hunt</h1>
        <p style={{ fontSize: "0.875rem", marginBottom: "1rem", color: "#9ca3af" }}>
          👆 Drag to rotate • 🤏 Pinch to zoom (auto-resets) • 👆 Tap to say hi
        </p>
        <button
          onClick={() => setStart(true)}
          style={{ padding: "0.75rem 1.5rem", backgroundColor: "#10b981", borderRadius: "0.5rem", border: "none", color: "white", cursor: "pointer", fontSize: "1rem" }}
